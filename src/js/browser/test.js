/* global require */
const browserData = require("./index.js").default;
...
for(const test of expected_results){
  const report = browserData(test.userAgent);
  if(test.report.browser.name !== report.browser.name
		|| test.report.browser.version !== report.browser.version
		|| test.report.os.name !== report.os.name
		|| test.report.os.version !== report.os.version){
